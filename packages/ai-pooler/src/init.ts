...
import { existsSync, mkdirSync, readFileSync, readdirSync, writeFileSync, chmodSync, unlinkSync, copyFileSync, rmSync } from 'fs';
...
export async function init(options: InitOptions = {}): Promise<void> {
  ...
  if (options.encryptionKey) {
    ...
    // Ask user: custom or auto-generated?
    const choice = await prompt(
      'Encryption password (encrypts PII locally before sending):\n' +
      '  1) Generate a secure random password (recommended)\n' +
      '  2) Enter your own password\n' +
      'Choose [1/2]: '
    );

    if (choice.trim() === '2') {
      encryptionPassword = await promptPassword('Enter your encryption password (min 8 characters):\n> ');
      if (encryptionPassword.length < 8) {
      ...
      console.log(green('\n+ Secure encryption password generated'));
      console.log(dim('Retrieve it anytime with: npx @rulecatch/ai-pooler config --show-key\n'));
    }
  } else {
    encryptionPassword = randomBytes(32).toString('base64');
    autoGeneratedKey = true;
  ...
  // 6b. Clear stale session, backpressure, and buffer from previous config
  // Without this, switching API keys or regions causes "Session expired" loops
  const staleFiles = ['.session', '.backpressure-state'];
  for (const f of staleFiles) {
    const p = join(RULECATCH_DIR, f);
    if (existsSync(p)) unlinkSync(p);
  }
  // Clear buffered events (they were signed with the old session token)
  if (existsSync(BUFFER_DIR)) {
    for (const f of readdirSync(BUFFER_DIR)) {
      if (f.endsWith('.json')) unlinkSync(join(BUFFER_DIR, f));
    }
  }
